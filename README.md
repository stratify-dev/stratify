...
- uses: stratify-dev/stratify@v0.3.0
  with:
    path: .
    fail-on: warning
```

The step fails the job when at least one finding meets or exceeds the `fail-on` threshold. Pin to a released tag for stable runs, for example `@v0.3.0`. `@main` tracks the latest. The Action downloads a prebuilt binary, so it starts in seconds.
